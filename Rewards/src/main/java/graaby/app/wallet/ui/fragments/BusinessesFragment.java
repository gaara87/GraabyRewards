...
public class BusinessesFragment extends BaseFragment
    ...
    private void loadMarkersFromDB(int brandID) {
        ...
        Log.d(TAG, "ORM initialized status - " + ormService);
        if (brandID == Helper.DEFAULT_NON_BRAND_RELATED)
            iterator = ormService.getAllOutlets().listIterator();
        else
            iterator = ormService.getAllOutletsForBrand(brandID).listIterator();
        ...
        mCompositeSubscriptions.add(mBusinessService.getOutletsAroundLocation(request)
                ...
                .subscribe(new CacheSubscriber<OutletsResponse>(getActivity(), mSwipeRefresh, false) {
                    ...
                    public void onSuccess(OutletsResponse result) {
                        ormService.addOutletsCollection(result.outlets);
